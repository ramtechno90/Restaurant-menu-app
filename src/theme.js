export const COLORS = {
  primary: '#FC8019', // Swiggy's orange
  secondary: '#282C3F', // A dark color for text and headers
  white: '#FFFFFF',
  black: '#000000',
  lightGray: '#F7F7F7',
  gray: '#A9A9A9',
  darkGray: '#535665',
  success: '#4CAF50',
  danger: '#F44336',
};

export const SIZES = {
  // global sizes
  base: 8,
  font: 14,
  radius: 12,
  padding: 24,

  // font sizes
  h1: 30,
  h2: 22,
  h3: 16,
  h4: 14,
  body1: 30,
  body2: 22,
  body3: 16,
  body4: 14,
};

export const FONTS = {
  h1: { fontFamily: 'sans-serif', fontSize: SIZES.h1, lineHeight: 36 },
  h2: { fontFamily: 'sans-serif', fontSize: SIZES.h2, lineHeight: 30 },
  h3: { fontFamily: 'sans-serif-medium', fontSize: SIZES.h3, lineHeight: 22 },
  h4: { fontFamily: 'sans-serif-medium', fontSize: SIZES.h4, lineHeight: 22 },
  body1: { fontFamily: 'sans-serif', fontSize: SIZES.body1, lineHeight: 36 },
  body2: { fontFamily: 'sans-serif', fontSize: SIZES.body2, lineHeight: 30 },
  body3: { fontFamily: 'sans-serif', fontSize: SIZES.body3, lineHeight: 22 },
  body4: { fontFamily: 'sans-serif', fontSize: SIZES.body4, lineHeight: 22 },
};

const appTheme = { COLORS, SIZES, FONTS };

export default appTheme;
